...
| | 类别 | 原因短语 |
| --- | --- | --- |
| 1XX | Informational（信息响应） | 接收的请求且继续处理，临时响应 |
| 2XX | Success（成功） | 请求正常处理完毕 |
| 3XX | Redirection（重定向） | 需要进行附加操作以完成请求 |
| 4XX | Client Error（客户端错误） | 客户端无法处理请求 |
...
1XX 信息响应，接收到请求并且继续处理，属于 **临时响应**

- 101

    协议切换，客户端要求服务器切换协议并且服务器已确认切换
...
- 201
    ...
    请求已经被实现，有新的资源依据请求的需要而建立，且其 URI 随 `Location` 头信息返回
...
- 403 Forbidden
    ...
    表明对请求资源的访问被服务器拒绝，服务器接收到请求，但拒绝执行
...
- 502
    ...
    作为网关或代理的服务器在尝试执行请求时，从上游服务器接收到无效响应
