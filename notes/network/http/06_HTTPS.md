...
由于 HTTP 以明文方式发送请求正文，并且不提供任何方式的数据加密，**可截取浏览器和网站服务器之间的传输报文，从而读取传输的信息（身份证号，登录密码等）**

HTTPS 在 HTTP 基础上加入安全套接层（SSL）和传输层安全（TLS），依靠证书验证服务器身份，并为浏览器和服务器之间的通信加密

- HTTP 协议不需要证书，而 HTTPS 需要 CA 申请证书（收费）
- HTTP 超文本传输协议，以明文方式传输，HTTPS 具有安全性的 SSL 加密传输协议
- HTTP 和 HTTPS 使用的是完全不同的连接方式：HTTP（80）、HTTPS（443）
- HTTP 连接无状态，HTTPS 由 SSL 和 HTTP 构建的可进行加密传输、身份验证的网络协议，也是无状态

SSL 协议及其继任者 TLS 协议都是为网络通信提供安全服务的安全协议

- 认证客户端和服务器，确保数据发送到正确地方
- 加密数据，防止数据中途被窃取
