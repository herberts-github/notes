...
Cookie 由服务器发给客户端的特殊信息，将信息以文本文件形式存放在客户端，客户端每次向服务器发送请求都携带特殊信息

- Cookie：通过在客户端存储身份信息的方式与服务器保持状态
- Session：通过服务器保持状态，Session 对象存储特定用户会话所需的属性及配置信息

> 当用户在页面之间跳转时，存储在 Session 对象的变量不会丢失，在整个用户会话一直存在
>
> 如果用户在访问网页时没有 Session，则服务器将自动创建 Session 对象，当会话过期或放弃后，服务器也会终止会话

### 生存周期与持久性

在生成时会指定 `Expire` 值，Cookie 的生存周期，超过周期后就被清除

> 将生存周期设置 0 或 负数，当关闭浏览器时，Cookie 就会被浏览器清除，下次打开再生成新的 Cookie 值
> 
> **常用于金融类网站，需在浏览器关闭时立即清除用户身份信息，确保信息安全**

### 属性和结构

由变量名称和值组成，其中变量既有**标准变量**也有**自定义格式变量**
...
- Expires：日期变量，格式为 `DD-MM-YY HH:MM:SS GMT`
- Path（可选）：规定服务器上哪些路径下的页面可获取服务器设置的 Cookie
  - 如果输入的 URL 路径部分从第一个字符开始包含 Path 属性定义的字符串，浏览器就认为通过检查
  - 如果为 `/` ，则服务器所有的 WWW 资源均可读取该 Cookie
  - 如果省略，属性值则为服务器传给浏览器的资源路径名
- Domain（可选）：可访问 Cookie 信息域名并且是只写变量，规定了域中的服务器可读取浏览器所存的 Cookie
  - 如果省略，Cookie 属性值为该服务器的域名
  - 如果设为 taobao.com，所有类似 `m.taobao.com` 或 `taobao.com` 页面就可访问 Cookie 信息
- SECURE：如果标记该变量，表明只有当浏览器和服务器之间通信协议为加密协议(HTTPS)时，浏览器才会向服务器提交相应 Cookie

> Cookie 属性可搭配使用，比如借助 Domain 和 Path 两个属性来控制 Cookie 被访问的范围
...
> 除了服务器在响应头使用 `Set-Cookie` 头域向客户端发送 Cookie 之外，
> 
> **也可通过 JavaScript 代码设置 Cookie**
